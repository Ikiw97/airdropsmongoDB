...
const PrivateKeyGeneratorSecure = () => {
  ...
  useEffect(() => {
    ...
    const detectDevTools = () => {
      // Increased threshold to 300px to avoid false positives from taskbar, browser chrome, zoom, etc.
      const threshold = 300;
      const widthDiff = window.outerWidth - window.innerWidth;
      const heightDiff = window.outerHeight - window.innerHeight;

      if (widthDiff > threshold || heightDiff > threshold) {
        // Require multiple consecutive detections to confirm (debounce)
        devToolsDetectedCount++;
        if (devToolsDetectedCount >= 3 && generatedKeys.length > 0) {
          setGeneratedKeys([]);
        ...
        // Reset counter if not detected
        devToolsDetectedCount = 0;
      }
    };

    document.addEventListener('keydown', blockDevTools, true);
    const devToolsInterval = setInterval(detectDevTools, 1000); // Check every 1 second instead of 500ms
