  ...
  return (
    <div className="w-full mb-6 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* BITCOIN FEES */}
        <div className="p-4 rounded-xl border border-orange-500/20 bg-[#161b22] relative overflow-hidden group">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="text-orange-400 font-mono font-bold">{data.btcFees.fastestFee} sat/vB</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-400">30 min</span>
              <span className="text-yellow-400 font-mono font-bold">{data.btcFees.halfHourFee} sat/vB</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-400">1 hour</span>
              <span className="text-blue-400 font-mono font-bold">{data.btcFees.hourFee} sat/vB</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-400">Economy</span>
              <span className="text-green-400 font-mono font-bold">{data.btcFees.economyFee} sat/vB</span>
            </div>
          </div>
        </div>

        {/* ETHEREUM NETWORK */}
        <div className="p-4 rounded-xl border border-blue-500/20 bg-[#161b22] relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
          ...
          <div className="space-y-4">
            <div className="flex items-end gap-2">
              <span className="text-4xl font-bold text-gray-200">{data.ethGas.average || data.ethGas.propose}</span>
              <span className="text-sm text-gray-500 mb-1">Gwei</span>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between text-xs text-gray-400">
                <span>Safe Low</span>
                <span className="text-green-400">{data.ethGas.slow || data.ethGas.safe}</span>
              </div>
        ...
        {/* BITCOIN STATS */}
        <div className="p-4 rounded-xl border border-yellow-500/20 bg-[#161b22] relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center text-sm border-b border-gray-800 pb-2">
              ...
              <span className="text-green-400 font-mono">#{data.btcStats.height?.toLocaleString()}</span>
            </div>
            <div className="flex justify-between items-center text-sm border-b border-gray-800 pb-2">
              <span className="text-gray-400">Price</span>
              <span className="text-white font-mono">${data.btcStats.price?.toLocaleString()}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-400">Market Cap</span>
              <span className="text-gray-300 font-mono">{formatCurrency(data.btcStats.marketCap)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 rounded-xl border border-gray-800 bg-[#161b22]">
        <h3 className="text-purple-400 font-bold flex items-center gap-2 mb-4 uppercase tracking-wider text-sm">
          <Layers size={18} /> Top Chains by TVL
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {data.topChains.map((chain, index) => (
            <div key={chain.name} className="flex flex-col items-center bg-gray-900/50 p-3 rounded-lg border border-gray-800 hover:border-purple-500/50 transition-colors">
              <span className="text-2xl font-bold text-gray-500 mb-1">#{index + 1}</span>
              <span className="text-sm font-bold text-gray-200">{chain.name}</span>
              <span className="text-xs text-green-400 font-mono mt-1">{formatCurrency(chain.tvl)}</span>
            </div>
          ))}
      ...
      <div className="p-5 rounded-xl border border-gray-800 bg-[#161b22]">
        <h3 className="text-green-400 font-bold flex items-center gap-2 mb-4 uppercase tracking-wider text-sm">
          <Globe size={18} /> Network Health
        </h3>
        <div className="flex flex-wrap gap-6">
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-300 w-20">Bitcoin</span>
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-300 w-20">Ethereum</span>
            <div className="flex items-center gap-1.5 bg-green-500/10 px-2 py-1 rounded text-green-400 text-xs font-bold border border-green-500/20">
              <span className="w-1.5 h-1.5 bg-green-500 rounded-full"></span> Healthy
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-300 w-20">DeFi</span>
            <div className="flex items-center gap-1.5 bg-blue-500/10 px-2 py-1 rounded text-blue-400 text-xs font-bold border border-blue-500/20">
